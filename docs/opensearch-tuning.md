...
**已尝试但无效：search 改多进程（`ProcessPoolExecutor`）**。理论上多进程能完全绕开 GIL，
但实测（`MSEARCH_PROCESS_WORKERS=8-16`, `MSEARCH_PROCESS_CHUNK=1000-5000`）**没有超过
orjson+多线程的成绩**，部分配置反而更慢。原因：`orjson` 的编码本身是 C 扩展，执行期间会
**释放 GIL**，所以多线程调用 orjson 已经接近多核并行效果；而多进程额外付出了 numpy 数组
pickle 序列化 + IPC 传输的成本，抵消了理论收益。**结论：orjson 已经解决了 GIL 瓶颈，无需
上多进程的复杂度**。此路径已回退，仅记录供后续参考。

---

## 7. 大规模（亿级 / 十亿级）的架构建议

### 7.1 退化是温和的，但吞吐天花板是真的
...
### 7.2 7B 的正解：把"插入"和"查询"解耦（两阶段离线去重）
